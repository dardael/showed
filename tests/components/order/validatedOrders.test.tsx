...
describe('NewOrders Component', () => {
    ...
    it('renders loading state initially', async () => {
        ...
        render(<ValidatedOrders />);

        expect(screen.getByText('Loading...')).toBeInTheDocument();
        await waitFor(() =>
            expect(getOrders).toHaveBeenCalledWith(OrderState.VALIDATED)
        );
    });
    ...
    it('handles empty orders gracefully', async () => {
        ...
        await waitFor(() => {
            expect(
                screen.getByText('Aucune commande à réceptionner')
            ).toBeInTheDocument();
        });
    });
    ...
    it('validates an order', async () => {
        ...
        const validateButton = screen.getAllByText('Valider la réception')[0];
